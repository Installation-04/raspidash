...
export const api = {
  ...
  updateWidget: (id: string, data: any) => put<any>(`/settings/widgets/${id}`, data),
  deleteWidget: (id: string) => del(`/settings/widgets/${id}`),
  weather: (lat: string, lon: string, unit: string) => get<any>(`/weather/current?lat=${lat}&lon=${lon}&unit=${unit}`),
